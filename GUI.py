...
class GUI:
     def __init__(self, sm: RTSSM.RTSMachine) -> None:       
          ...
          if self.sm.current_state.id == "moving":
               self.label: tk.Label = tk.Label(self.root, text="Current State: " + self.sm.current_state.id, font=('Arial', 20), fg="#7cb46b")
               self.label.pack()
          else: 
               self.label: tk.Label = tk.Label(self.root, text="Current State: " + self.sm.current_state.id, font=('Arial', 20), fg="#000000")
          ...
          self.startbtn: tk.Button = tk.Button(self.frame, text="Start", font=('Arial', 18), command=self.start_robot)
    ...
    # Changes label text and color to correspond to the current state
     def update_label(self):
          if self.sm.current_state.id == "moving":
               self.label.config(text="Current State: " + self.sm.current_state.id, fg="#7cb46b")
               self.root.update()
          elif self.sm.current_state.id == "stopping":
               self.label.config(text="Current State: " + self.sm.current_state.id, fg="#f1cf4c")
               self.root.update()
          elif self.sm.current_state.id == "stopped":
               self.label.config(text="Current State: " + self.sm.current_state.id, fg="#d60909")
               self.root.update()
          else:
               self.stopbtn: tk.Button = tk.Button(self.frame, bg="#FF0000", text="Stop", font=('Arial', 18), command=self.stop_robot)
               self.stopbtn.grid(row=0, column=1, padx=10, pady=10)
